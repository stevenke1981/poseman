...
- **移動模式**：直接拖曳人物／物品在較大場景範圍內移動（X/Z ±50、Y -20..50，可離地或略低於地面）；Shift 鎖定水平、Alt 只改高度；也可用座標滑桿、方向鍵／Q E 微調，或 TransformControls gizmo 沿任一軸／平面精確移動；地面顯示軸色十字基準（X紅/Z藍/Y綠）＋垂直高度線與即時座標標籤
...
- **鍵盤快捷鍵**：M 移動模式、P 預覽、G 格線、Delete 刪除選取物品、Esc 取消選取／離開移動、1/2/3 選骨盆／胸腔／頭部、移動模式方向鍵／PageUp／PageDown／Q／E 微調位置（Shift 更細）、Ctrl+Z／Ctrl+Y 復原／重做（輸入框焦點時不觸發）
...
| 關節上拖曳 | 旋轉該關節 |
| 移動模式＋拖曳人物/物品 | 較大範圍自由移動（含高度）；Shift 水平、Alt 垂直；gizmo／滑桿／方向鍵可精確移動 |
| 空白處拖曳 | 旋轉視角 |
| 滾輪／雙指 | 縮放 |
